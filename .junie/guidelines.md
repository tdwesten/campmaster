...
## Translations
- The source of the application translations is the `resources/lang/` directory
- The source language is English but add also new translations for other languages
- NEVER update the lingua.js file directly, it's automatically generated by Vite
- Use the `trans()` helper with `const { trans } = useLingua();` in TypeScript files
- Use `__('key')` in Blade templates and PHP files
- Use `@lang('key')` in Blade templates for echoing translations
- Always use translation keys, never hardcode text
- Only update the EN translations files

## Routes and Controllers
- Create a controller for each resource, so for example `BookingsController` or `TenantsController`
- Use the `Route::resource()` helper to create routes for each resource

## Roles

- Use Spatie's Laravel Permissions package for role-based access control
- Roles: Admin, Manager
